          ...
          <div class="card-body">
            ...
            @if(in_array($application->appliedStream->stream_id, [4,6,8]))
              @foreach($major_subjects as $key => $subject)
                ...
                @foreach($categories as $category)
                {{$category->name}} : 
                  @if(array_key_exists($subject->id,$seat_details['allocated_applications']))
                    @if(array_key_exists($category->id,$seat_details['allocated_applications'][$subject->id]))
                    {{$seat_details['allocated_applications'][$subject->id][$category->id]}}/{{$seat_details['reservations'][$subject->id][$category->id]}}
                    @endif
                  @else
                  0 
                  @endif 
